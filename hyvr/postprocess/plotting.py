...
def cross_section_pcolor(model, field, y=None, x=None, log=False, xlim_y=None, ylim_y=None,
    ...
    for yi in y:
        ...
        if 0 <= y_index and y_index < model.grid.ny:
            ...
            ax.set_xlabel('x')
            ax.set_ylabel('z')
            if xlim_y is not None: ax.set_xlim(xlim_y)
            if ylim_y is not None: ax.set_ylim(ylim_y)
            ax.set_aspect('equal')
    ...
    for xi in x:
        ...
        if 0 <= x_index and x_index < model.grid.nx:
            ...
            ax.set_xlabel('y')
            ax.set_ylabel('z')
            if xlim_x is not None: ax.set_xlim(xlim_x)
            if ylim_x is not None: ax.set_ylim(ylim_x)
            ax.set_aspect('equal')
            fig.colorbar(im, ax=ax)
            plt.show(fig)
        else:
